...
public class HallucinationFollowGoal extends FollowOwnerGoal {
    ...
    public HallucinationFollowGoal(TamableAnimal pTamable, double pSpeedModifier, float pStartDistance, float pStopDistance, boolean pCanFly) {
        ...
        if (!(pTamable.getNavigation() instanceof GroundPathNavigation) && !(pTamable.getNavigation() instanceof FlyingPathNavigation)) {
            throw new IllegalArgumentException("Unsupported mob type for HallucinationFollowGoal");
        }
    }
    ...
    public void tick() {
        ...
        if (--this.timeToRecalcPath <= 0) {
            ...
            if (this.tamable.distanceToSqr(this.owner) >= 10000) {
    ...
    private void teleportToOwner() {
        ...
        for(int $$1 = 0; $$1 < 10; ++$$1) {
            int $$2 = this.randomIntInclusive(-5, 5);
            int $$3 = this.randomIntInclusive(-2, 2);
            int $$4 = this.randomIntInclusive(-5, 5);
    ...
    private boolean maybeTeleportTo(int pX, int pY, int pZ) {
        if (Math.abs((double)pX - this.owner.getX()) < 2.0 && Math.abs((double)pZ - this.owner.getZ()) < 2.0) {
            ...
            this.tamable.moveTo((double)pX + 0.5, pY, (double)pZ + 0.5, this.tamable.getYRot(), this.tamable.getXRot());
            this.navigation.stop();
            return true;
        }
    }
